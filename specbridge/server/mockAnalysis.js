// Server-side mock analysis for /api/translate

export const MOCK_ANALYSIS_RESULT = {
  category: '개발 · IT',
  terms: [
    { term: 'API', description: '프로그램이나 서비스가 서로 데이터를 주고받을 수 있도록 만든 연결 방법' },
    { term: '서버', description: '데이터를 저장하거나 처리해서 사용자에게 제공하는 시스템' },
    { term: '프론트엔드', description: '사용자가 직접 보고 조작하는 웹사이트나 앱의 화면 부분' },
    { term: '렌더링', description: '데이터나 코드를 실제 화면에 표시하는 과정' },
    { term: 'UI', description: '사용자가 직접 보는 화면과 버튼 등의 인터페이스' },
    { term: 'UX', description: '사용자가 서비스를 이용하면서 느끼는 전체적인 경험' },
  ],
  easySentence:
...
export function buildMockAnalysis(text, selectedCategory, selectedLevel) {
  ...
  const levelVariants = {
    simple: '핵심만 요약하면, 데이터를 받아 처리해 화면에 보여주고 UX를 개선해야 합니다.',
    basic: MOCK_ANALYSIS_RESULT.easySentence,
    detail:
      '외부 데이터 수집 → 서버 처리 → 프론트 렌더링의 흐름입니다. 화면 자체는 괜찮지만, 실제 사용자가 느끼는 경험을 개선하는 작업(UX)이 필요합니다.',
  }

  const easySentence = levelVariants[selectedLevel] || levelVariants.basic
